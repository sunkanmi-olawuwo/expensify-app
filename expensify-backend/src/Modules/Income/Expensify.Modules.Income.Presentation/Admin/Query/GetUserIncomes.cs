...
using Expensify.Modules.Income.Application.Admin.Query.GetUserIncomes;
...
public sealed class GetUserIncomes : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        app.MapGet(RouteConsts.AdminIncome, async (IMediator mediator, HttpContext context, Guid userId, [AsParameters] GetUserIncomesRequest request) =>
            {
                Result<IncomePageResponse> result = await mediator.Send(
                    new GetUserIncomesQuery(
                        userId,
                        request.Period,
                        request.Source,
                        request.Type,
            ...
            .WithName(nameof(GetUserIncomes))
            .WithTags("Income")
            .WithSummary("Gets user incomes for admins.")
            .WithDescription("Returns paged income records for a selected user and period.")
            .RequireAuthorization(IncomePolicyConsts.AdminReadPolicy)
            .Produces<IncomePageResponse>(StatusCodes.Status200OK);
    }

    public sealed class GetUserIncomesRequest
    {
