...
internal sealed class GetInvestmentsQueryHandler(IDbConnectionFactory dbConnectionFactory)
    : IQueryHandler<GetInvestmentsQuery, InvestmentAccountsPageResponse>
{
    private const int DefaultPage = 1;
    private const int DefaultPageSize = 20;
    private const int MaxPageSize = 100;

    public async Task<Result<InvestmentAccountsPageResponse>> Handle(GetInvestmentsQuery request, CancellationToken cancellationToken)
    {
        ...
        List<Row> rows = (await connection.QueryAsync<Row>(pageSql, new { PageSize = pageSize, Offset = offset })).AsList();
