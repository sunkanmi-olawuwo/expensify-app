...
using Expensify.Common.Infrastructure.Serialization;
    ...
    ILogger<ProcessOutboxJob> logger) : IJob
{
    ...
    private async Task<IReadOnlyList<OutboxMessageResponse>> GetOutboxMessagesAsync(IDbConnection connection, IDbTransaction transaction)
    {
        string sql =
            ...
            FROM investments.outbox_messages
        ...
        return (await connection.QueryAsync<OutboxMessageResponse>(sql, transaction: transaction)).ToList();
    }
        ...
        Exception? exception)
    {
        const string sql =
            """
            UPDATE investments.outbox_messages
