...
namespace Expensify.Modules.Investments.Infrastructure.Outbox;
    ...
    where TDomainEvent : IDomainEvent
{
    ...
    private static async Task<bool> OutboxConsumerExistsAsync(DbConnection dbConnection, OutboxMessageConsumer outboxMessageConsumer)
    {
        const string sql =
            """
            SELECT EXISTS(
                SELECT 1
                FROM investments.outbox_message_consumers
    ...
    private static async Task InsertOutboxConsumerAsync(DbConnection dbConnection, OutboxMessageConsumer outboxMessageConsumer)
    {
        const string sql =
            """
            INSERT INTO investments.outbox_message_consumers(outbox_message_id, name)
