    ...
    where TIntegrationEvent : IIntegrationEvent
{
    ...
    private static async Task<bool> InboxConsumerExistsAsync(DbConnection dbConnection, InboxMessageConsumer inboxMessageConsumer)
    {
        const string sql =
            """
            SELECT EXISTS(
                SELECT 1
                FROM investments.inbox_message_consumers
    ...
    private static async Task InsertInboxConsumerAsync(DbConnection dbConnection, InboxMessageConsumer inboxMessageConsumer)
    {
        const string sql =
            """
            INSERT INTO investments.inbox_message_consumers(inbox_message_id, name)
