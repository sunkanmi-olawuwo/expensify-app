    ...
    ILogger<ProcessInboxJob> logger) : IJob
{
    ...
    private async Task<IReadOnlyList<InboxMessageResponse>> GetInboxMessagesAsync(IDbConnection connection, IDbTransaction transaction)
    {
        string sql =
            ...
            FROM investments.inbox_messages
        ...
        return (await connection.QueryAsync<InboxMessageResponse>(sql, transaction: transaction)).AsList();
    }
        ...
        Exception? exception)
    {
        const string sql =
            """
            UPDATE investments.inbox_messages
