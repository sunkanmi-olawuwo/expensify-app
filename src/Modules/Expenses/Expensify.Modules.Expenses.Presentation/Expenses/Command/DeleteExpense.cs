...
public sealed class DeleteExpense : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        app.MapDelete(RouteConsts.ExpenseById, async (IMediator mediator, ClaimsPrincipal claims, Guid expenseId) =>
            {
                Result result = await mediator.Send(new DeleteExpenseCommand(claims.GetUserId(), expenseId));
